        ...
        pipedStream.on('close', function () {
            if (self.opts.times) {
                const filedate = new Date(
                    ((entry.lastModifiedDate >> 9) & 0x7f) + 1980,
                    ((entry.lastModifiedDate >> 5) & 0xf) - 1,
                    entry.lastModifiedDate & 0x1f,
                    (entry.lastModifiedTime >> 11) & 0x3f,
                    (entry.lastModifiedTime >> 5) & 0x3f,
                    (entry.lastModifiedTime & 0x3f) * 2);
                const mtime = Math.floor(filedate.getTime() / 1000);
                fs.utimes(destPath, mtime, mtime, function (error) {
